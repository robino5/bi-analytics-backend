...
class RMWiseClientDetailOrm(BaseOrm):
    ...
    branch_code: Mapped[int] = mapped_column(Numeric(10, 0), primary_key=True)
    ...
    tpv: Mapped[float] = mapped_column("TPV", Numeric(34, 2))
    cv: Mapped[float] = mapped_column("CV", Numeric(34, 2))
    int_amount: Mapped[float] = mapped_column("Int_Amount", Numeric(34, 2))
    available_cash_balance: Mapped[float] = mapped_column(
        "Available_cash_bal", Numeric(34, 2)
    )
    loan_balance: Mapped[float] = mapped_column("Loan_bal", Numeric(34, 2))
    equity: Mapped[float] = mapped_column("Equity", Numeric(34, 2))
    exposure_on_equity: Mapped[float] = mapped_column(
        "exposure_on_Equity", Numeric(34, 2)
    )
    daily_turnover: Mapped[float] = mapped_column("Daily_TurnOver", Numeric(34, 2))
    weekly_turnover: Mapped[float] = mapped_column("Weekly_TurnOver", Numeric(34, 2))
    fortnightly_turnover: Mapped[float] = mapped_column(
        "Fortnightly_TurnOver", Numeric(34, 2)
    )
    monthly_turnover: Mapped[float] = mapped_column("Monthly_TurnOver", Numeric(34, 2))
